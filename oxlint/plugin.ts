...
const noClasses = defineRule({
  meta: {
    type: "problem",
    docs: { description: "Plain consts and free functions instead of classes." },
    messages: { noClass: "Use plain consts and free functions, not classes." },
  },
  createOnce(context) {
    const report = (node: Node) => context.report({ node, messageId: "noClass" });
...
const noConditionalAwaitFallback = defineRule({
  meta: {
    type: "suggestion",
    docs: { description: "No `cond ? await x : []` ternaries; branch with an early return or a guard clause." },
    messages: { noTernary: "Avoid `cond ? await x : <fallback>`; use an early return or a guard clause." },
  },
  createOnce(context) {
    return {
      ConditionalExpression(node) {
        ...
        const hasEmpty = branches.some(
          b => (b.type === "ArrayExpression" && b.elements.length === 0) || (b.type === "Literal" && b.value === null) || (b.type === "Identifier" && b.name === "undefined"),
        );
